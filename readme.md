...
- Django-ninja는 도큐먼트 정보가 부족하여 부족한 기능을 직접 개발해야한다는 단점이 있음, 기업에서 사용하기에는 아직 부족한 점이 많으므로 DRF로 해당 프로젝트의 내용을 토대로 새롭게 개발하는 방향도 나쁘지 않다고 생각됨
...
- 유닛 테스트를 위한 Pytest library 도입

  - 기존 테스트 모듈이 내장되어있음에도 불구하고, 실행이 순차적으로 진행되는 메커니즘 때문에 규모가 커지는 경우 테스트가 그만큼 느려진다는 단점이 존재함
  - 반면 pytest는 멀티 실행을 지원하므로 이러한 문제를 해결할 수 있음


```shell
## 테스트를 위해 tests라는 디렉토리 내에 테스트할 정보들을 입력함

# 디렉토리 내의 모든 테스트를 수행하려는 경우
$ python -m pytest tests

# View 옵션 추가 시
$ python -m pytest -v tests

# 하나의 파일에 한해서 테스트를 수행하려는 경우
$ pytests file_name.py
...
- pytest를 활용한 테스트 기반의 빌드 방식 추가
  - pytest를 통해 테스트 커버리지 60 ~ 70%를 유지할 예정
  - 대부분이 API 기능이므로 API에 대한 테스트 진행 예정
  - 작업 중 -